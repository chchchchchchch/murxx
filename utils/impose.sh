...
# -------------------------------------------------------------- #
  PDFNAME=`basename $PDF | sed 's/\.pdf$//'` #
  ...
  WXH=`pdfinfo $PDF       | #
       ...
       sed 's/x/ /g'      | #
       sed 's/[a-z]//g'`    #
  W=`echo $WXH | cut -d " " -f 1`;H=`echo $WXH | cut -d " " -f 2`
  W=`expr $W \* 2 - $CROPSUM`;H=`expr $H - $CROPSUM`
  NUMPAGES=`pdfinfo $PDF | grep "^Pages" | sed 's/[^0-9]*//g'`
  PA="$NUMPAGES";PB="1";PC="2";PD=`expr $NUMPAGES - 1`
# -------------------------------------------------------------- #
  printf "IMPOSE PAGES "
...
# -------------------------------------------------------------- #
  PAGES=`echo $PAGES | sed 's/^,//' | sed 's/ //g'`
...
# -------------------------------------------------------------- #
  PDFNAME=`basename ${TMP}.pdf | sed 's/\.pdf//'`
  GSCMD="gs -sDEVICE=tiffsep -dNOPAUSE -dBATCH -dSAFER -r600 \
            -sOutputFile=$PDFNAME.%04d.tif $PDFNAME.pdf"
  GSCMD=`echo $GSCMD | tr -s ' '`

  echo ""
  echo -e "\nTO SEPARATE CMYK PLATES RUN" | tee -a ${TMP}.info
  echo "-> $GSCMD" | tee -a ${TMP}.info
